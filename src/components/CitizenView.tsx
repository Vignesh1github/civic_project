...
import { MapPin, Camera, Loader2, UploadCloud } from 'lucide-react';

interface CitizenViewProps {
  user: User;
  complaints: Complaint[];
  // Accept raw data payload
  onAddComplaint: (data: { title: string, description: string, imageBase64: string | null, location: LocationData | null }) => Promise<void>;
  onLogout: () => void;
}
...
const CitizenView: React.FC<CitizenViewProps> = ({ user, complaints, onAddComplaint, onLogout }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send raw data to App component -> Backend
                ...
                {image ? (
                  <div className="relative w-full h-32 group">
                    ...
                    <button 
                      ...
                      className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 text-xs opacity-90 hover:opacity-100"
                    >
                      Remove
                    </button>
                  </div>
                  ...
                  <>
                    ...
                    <span className="text-sm text-gray-500">Tap to upload photo (Optional)</span>
                  </>
                )}
              </div>
